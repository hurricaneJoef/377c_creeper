...
void mecanum_move(float forward, float rotation, float strafe, bool dist=false, float velocitymult= 30){
  if(dist){
    ...
    switch(maxnum){
      ...
      leftfront.startRotateFor(vex::directionType::fwd,  (-rotation-forward-strafe),vex::rotationUnits::rev,2*(-rotation-forward+strafe)*velocitymult,vex::velocityUnits::rpm);
